...
* Company mode
"COMPlete ANYthing" mode (Company mode) is an auto-completion framework designed for text and coding.
#+begin_src elisp
(use-package company
  :ensure t
  :init
...
Certain macros make programming neater and easier to write.
#+begin_src emacs-lisp
...
* Custom Settings
QoL changes to make the environment more intuitive visually and navigation-wise
#+begin_src emacs-lisp
...
* Expand Region
Allows for expansion by semantic units. Paired nicely with pending delete mode
#+begin_src emacs-lisp
...
* Magit
Magit is considered one of the best git clients, so we use that to manage our repos
Create a new repo with the bash command: gh repo create newRepoName --source=. --public --push
#+begin_src emacs-lisp
(use-package magit
  :ensure t
...
* Multiple Cursors
Multiple Cursors is a popular plugin that allows for multiple editing areas
#+begin_src emacs-lisp
...
* Spell Checker
Aspell (when installed on the system with the package manager) can be used via flycheck for spell checking. Must have langtool-server.jar for grammar
#+begin_src emacs-lisp
...
* Tsoding-inspired additions
Certain features, such as "Simple C" mode, and an astyle function to help reformat
- Make sure to download simpc-mode.el from his github and add to the init.el
#+begin_src emacs-lisp
...
* Yasnippet
Creates snippets for boilerplate specifically within programming modes
#+begin_src emacs-lisp
